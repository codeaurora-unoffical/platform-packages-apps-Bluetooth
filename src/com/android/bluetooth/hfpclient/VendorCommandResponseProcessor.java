...
class VendorCommandResponseProcessor {
    ...
    public boolean sendCommand(String atCommand, BluetoothDevice device) {
        if (device == null) {
    ...
    public boolean processEvent(String atString, BluetoothDevice device) {
        ...
        broadcastVendorSpecificEventIntent(eventCode, atString, device);
        logD("process vendor event " + eventCode + ", "
                + atString + " for device" + device);
        return true;
    }

    private void broadcastVendorSpecificEventIntent(String vendorEventCode,
