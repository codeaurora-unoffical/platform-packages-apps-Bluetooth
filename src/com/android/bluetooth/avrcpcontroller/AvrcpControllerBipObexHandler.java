...
class AvrcpControllerBipObexHandler extends Handler {
    ...
    private final String THUMB_PREFIX = "AVRCP_BIP_THUMB_";
    /** Default prefix for image files stored on external storage */
    private final String IMG_PREFIX = "AVRCP_BIP_IMG_";
    /** Default Transformation supported for Image retreival */
    private final String DEF_TRANSFORMATION_SUPPORTED = "stretch";
            ...
            if (file != null) {
                for (int i = 0; i < file.length; i++) {
                    if (file[i].getName().startsWith(fileToBeDeletedPrefix)) {
                        if (DBG) {
                            Log.d(TAG, "file is deleted  @ " + file[i].getAbsolutePath());
                        }
                        file[i].delete();
                    }
                }
            ...
            if (file != null) {
                for (int i = 0; i < file.length; i++) {
                    if (file[i].getName().startsWith(fileToBeSearch)) {
                        if (DBG) {
                            Log.d(TAG, "file is present @ " + file[i].getAbsolutePath());
                        }
    ...
    public String getLinkedThumbnail(String imgHandle){
        ...
        if (DBG) {
            Log.d(TAG, "getLinkedThumbnil: localPath " + localPath + " ImgHandle: "
                + imgHandle);
        }
        if (localPath == null) {
            ...
            if (VDBG) {
                Log.v(TAG, "getLinkedThumbnil: already exists at localPath " + localPath );
            }
    ...
    private Bundle getImage(String imgHandle, String encoding, String pixel, long maxSize) {
        ...
        if (DBG) {
            Log.d(TAG, "getImage: localPath " + localPath + " maxSize: "
                + maxSize + " Pixel: "+ pixel);
        }
        if (localPath == null) {
    ...
    private AvrcpBipImgDescriptor getPreferredImgDescriptor(AvrcpBipGetImageProperties prop) {
        if (prop.mReqImgDesc != null && prop.mResImgDesc != null) {
            ...
            for (int i = 0; i < prop.mResImgDesc.size(); i++) {
                ...
                if (i == 0) {
                    /* storing native format for image, in case if UI preferance does not match.*/
                ...
                if ((prop.mReqImgDesc.mEncoding).equals(mResImgDesc.mEncoding)) {
                    ...
                    if ((mResImgDesc.mPixel.indexOf("-") != -1)) {
                        if (DBG) {
                            Log.d(TAG, "GetPreferredImgDescriptor Range");
                        }
                        if (mResImgDesc.mPixel.indexOf("-") != mResImgDesc.mPixel.lastIndexOf("-")) {
                            Log.e(TAG, "GetPreferredImgDescriptor: res pixel field is invalid:"
                             ...
                             if (VDBG) {
                                 Log.v (TAG, "lowest range: " + range[0]);
                             }
                             if (range[0].indexOf("**") == -1) {
                                ...
                                if (VDBG) {
                                    Log.v(TAG, "reqWidth: " + reqWidth + "reqHeight: "
            ...
            if (resNativeImgDesc != null) {
                Log.e(TAG, "No valid match found, seclect native encoding ");
        ...
        Log.w(TAG, "Native encoding not found, return default request param");
    ...
    private byte[] createXmlFromImgDescriptor(AvrcpBipImgDescriptor imgDescriptor) {
        ...
        try {
            ...
            if (VDBG) {
                Log.v(TAG, "Image descriptor XML = " + sw.toString());
            }
    ...
    public abstract class AvrcpBipRequest {
        ...
        final public boolean isSuccess() {
            if (DBG) {
                Log.d(TAG, "isSuccess: " + mResponseCode);
            }
    ...
    final class AvrcpBipGetLinkedThumbnail extends AvrcpBipRequest {
        ...
        protected void readResponse(InputStream is) throws IOException {
            ...
            if (DBG) {
                Log.d(TAG, "mResponseCode: " + mResponseCode + "btDir: " + btDir);
            }
            if (btDir != null) {
                ...
                try {
                    if (DBG) {
                        Log.d(TAG,"readResponse: opening " + tmpFilePath +
                ...
                if (DBG) {
                    Log.d(TAG,"readResponse: writing to " + tmpFilePath);
                }
                while ((bytes_read = is.read(buffer)) != -1) {
                    if (VDBG) {
                        Log.v(TAG,"bytes_read: " + bytes_read);
                    }
                    ...
                    switch (event) {
                        case XmlPullParser.START_TAG:
                            if (imgDescParser.getName().equals("image-properties")) {
                                if (VDBG) {
                                    Log.v(TAG, "image-properties version: " +
