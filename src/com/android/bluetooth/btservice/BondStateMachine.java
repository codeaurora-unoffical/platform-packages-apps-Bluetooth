...
final class BondStateMachine extends StateMachine {
    ...
    static final int ADD_OOB_BOND_DEVICE = 11;
        ...
        public synchronized boolean processMessage(Message msg) {
            ...
            switch(msg.what) {
              ...
              case ADD_OOB_BOND_DEVICE:
                  ...
                  addOutOfBandBondDevice(dev, linkKey, msg.arg1, msg.arg2, true);
                  break;
              case REMOVE_BOND:
                  removeBond(dev, true);
                  break;
    ...
    private boolean addOutOfBandBondDevice(BluetoothDevice dev, String linkKey, int linkKeyType, int pinLen,
        ...
        if (dev.getBondState() == BluetoothDevice.BOND_NONE) {
            ...
            for (i = 0; i < linkKey.length(); i++) {
                key[j] = (byte) Integer.parseInt(linkKey.substring(i, i + 2), 16);
                j++;
                i++;
            }
